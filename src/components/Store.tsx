...
type Props = {
  tags: string[]
  data: CollectionEntry<"store">[]
}

export default function Store({ data, tags }: Props) {
  const [filter, setFilter] = createSignal(new Set<string>())
  const [stores, setStores] = createSignal<CollectionEntry<"store">[]>([])

  createEffect(() => {
    setStores(data.filter((entry) => 
      ...
      <div class="col-span-3 sm:col-span-2">
        <div class="flex flex-col">
          <div class="text-sm uppercase mb-2">
            SHOWING {stores().length} OF {data.length} STORE
          </div>
          <ul class="flex flex-col gap-3">
            {stores().map((store) => (
              <li>
                <ArrowCard entry={store} />
              </li>
            ))}
          </ul>
        </div>
